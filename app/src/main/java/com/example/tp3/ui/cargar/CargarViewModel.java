...
public class CargarViewModel extends AndroidViewModel {
    ...
    private boolean validar(String codigo, String descripcion, String precioStr){
        ...
        if (!valido) {
            mMsj.setValue(mensajeBuilder.toString().trim()); // .trim() para quitar el último \n si solo hay un error
            return false;
        }

        try{
            double p = Double.parseDouble(precioStr.trim()); // Usar trim() aquí también
